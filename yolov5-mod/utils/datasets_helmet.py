...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # --ty 以保存显示的图片数量, 用于调试时查看
        self.showed_image_count = 0
...
# 显示图片
def show(image, boxes, self, image_id):
    # 图片显示数量 self.showed_image_count, 每轮只显示两个图片
    if self.showed_image_count > 2:
        return

    for b in boxes:
        image = cv2.rectangle(image, (b[0], b[1]), (b[2], b[3]), (255, 0, 0), 1)
    cv2.imwrite("mixup_%s.jpg" % image_id, image)
    self.showed_image_count += 1
